...
<?php
	...
	//checking whether a student is applicable for photocopy registartion
	if(registration_status_photocopy($user_ref_id))
	{  //checking students staus whether registered or Not Registered
	  if(photocopy_registration_stat($user_ref_id) == "Not registered")
	  {
		if(isset($_POST['rcourse_list']) ){
			...
			$exam_reg = register_photocopy($rcourse_list,$user_ref_id);
			if($exam_reg == "success"){
?>
...
<?php		
				header( "refresh:5; url=photocopy_reg.php" );		
			}
			else{
?>
	<script type="text/javascript">
...
?>
			<div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                            <h2>PHOTOCOPY REGISTRATION</h2>
                           
                        </div>
                        <div class="body">
                            <form id="wizard_with_validation" method="POST" action="photocopy_reg.php" >
							<h3>WELCOME</h3>
                                <fieldset>
...
<?php 
	...
	if($user_type == 3)
	{
		...
		$view_query = "SELECT `student`.`id`,`student`.`prn`,`student`.`name`,`student`.`current_semester`,`student`.`department`,`department`.`department_name` FROM `marks_th_ese`,`reval_registration`,`student` LEFT JOIN `department` ON `department`.`id` = `student`.`department`  WHERE `student`.`id`='$user_type_id' AND `reval_registration`.`stud_id` = '$user_type_id' AND `reval_registration`.`conf_flag` = '1' AND `marks_th_ese`.`stud_id` = '$user_type_id' AND `marks_th_ese`.`reval_final` > '0'";
...
<?php
	...
	WHERE
	  `course`.`id` = `course_assign`.`sub_id` AND `course_assign`.`sem` = `student`.`current_semester` AND `course_assign`.`branch` = `student`.`department` AND `student`.`id` = '304' AND `course_type`.`name` = 'Theory' AND `reval_registration`.`course_id` = `course`.`id` AND `reval_registration`.`stud_id` = '304' AND `reval_registration`.`conf_flag` = '1' AND `marks_th_ese`.`course_id` = `course`.`id` AND `marks_th_ese`.`reval_flag` = '1'";
...
?>
												<tr>
													<td><?php echo $ct++; ?></td>
													<td><input type="checkbox" id="<?php echo "c".$row['id'];?>" name="rcourse_list[]" class="chk-col-teal" value="<?php echo $row['id'];?>" required />
													...
													<td><?php echo "{$row['reval_final']}";?></td>
												</tr>
<?php
	}
?>
