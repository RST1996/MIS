<?php
	...
	switch ($exam) {
		...
		$query = "SELECT `student`.`id`,`student`.`prn`,`student`.`name`,`student`.`department`,`student`.`current_semester` FROM `student`,`course_assign`,`makeup_registration`,`ses_conf` WHERE `student`.`status` = 'ONGOING' AND `course_assign`.`sub_id` = '$course' AND `course_assign`.`sem` = `student`.`current_semester` AND `course_assign`.`branch` = `student`.`department` AND `student`.`department`='$department' AND `makeup_registration`.stud_id = `student`.`id` AND `makeup_registration`.`course_id` = `course_assign`.`sub_id` AND `makeup_registration`.`sesn_id` = `ses_conf`.`current_session` AND `ses_conf`.`id` = '1' AND `makeup_registration`.`conf_flag` = '1' ";
