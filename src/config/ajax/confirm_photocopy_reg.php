<?php
	...
	$sem  = $_POST['sem'];
		...
		$query = "SELECT DISTINCT `id`, `prn`, `name`,`department`, `current_semester`, `status` FROM `student`, `reval_registration`  WHERE `status` = 'ONGOING' AND `department`=$department  AND `current_semester` = $sem AND `reval_registration`.`conf_flag`='1' AND `reval_registration`.`challenge_flag` ='1' AND  `reval_registration`.`stud_id` = `id` ORDER BY `prn` ASC ";
		$sesn_id = get_sesn_id();
...
<?php
				...
				while ($row = mysqli_fetch_assoc($res)) {
					...
					$check = photocopy_conform_stat($row['id']);
					$courses = get_photocopy_courses($row['id']);
