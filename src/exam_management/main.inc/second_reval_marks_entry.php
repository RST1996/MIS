<?php
	if(isset($_POST['lock_marks']))
	{
		...
		$s = lock_ese_reval_marks($sesn_id,$course_id,$dept,$exam_type);
		if($s)
		{
?>
...
<?php
			header( "refresh:1; url=second_reval_marks_entry.php" );
		}
	}
    if(isset($_POST['marks_list']))
    {
        ...
        $result = sec_reval_marks_entry($stud_list,$course_id,$marks_list);
        if($result == "success")
        {
	   ...
	   <div class="row clearfix">	
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="card">
                    <div class="header">
                        <h2>
						Second Revaluation Marks Entry
						</h2>                            
                    </div>
                    <div class="body">
						<form method = "POST" action = "second_reval_marks_entry.php">
                            <fieldset>
<?php
    //var_dump($activity);
...
<script type="text/javascript">
    function show_stud_list()
    {
        ...
        if (exam_id != null && course != null && dept != null) 
        {
            ...
            xhttp.open("POST", "../config/ajax/second_reval_marks_entry.php", true);
            xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
<script type="text/javascript">
    function edit_marks(mark_id,act_id,id)
    {
        ...
        var act_string = "<input type=\"button\" class=\"btn bg-teal waves-effect\" value=\"UPDATE\" onclick=\"update_marks('mark"+id+"','"+id+"')\" /> &nbsp;&nbsp; <input class=\"btn bg-teal waves-effect\" type=\"button\" value=\"CANCEL\" onclick=\"show_stud_list()\" />";
...
<script type="text/javascript">
    function update_marks(input_id,id)
    {
        ...
        if(mark <= max_marks && mark >= 0)
        {
            ...
            xhttp.open("POST", "../config/ajax/update_sec_reval_marks.php", true);
            xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
<script type="text/javascript">
	function finalize()
	{
        ...
        xhttp.open("POST", "../config/ajax/finalize_second_reval_ese_marks.php", true);
        xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
