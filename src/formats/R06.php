...
if( isset($_GET['id']) && (isLogin() != null) )
{
		...
		else
		{
			...
			$data_query = "SELECT  `academic_year`.`academic_year` ,  `course`.`course_code` ,  `course`.`course_name` ,  `course`.`credits` ,  `course_assign`.`sem` ,  `course_assign`.`branch` , `pr_mks_scheme`.`ese` ,  `department`.`department_name` ,  `exam_session`.`session`,`year2sem_relation`.`year`
			...
			$fileName = 'R06 ' . $row2['course_code'] . '.pdf';
		...
		// Line break
			...
			$pdf->SetFont('');
			//while start
			$query = "SELECT `student`.`id`,`student`.`prn`,`student`.`name`,`student`.`department`,`student`.`current_semester`,`marks_pr_ese`.`marks`,`marks_pr_ese`.`ne_flag`,`marks_pr_ese`.`ab_flag`,`course`.`course_code`,`course`.`course_name` FROM `student`,`course_assign`,`course`,`exam_registration`,`ses_conf`,`marks_pr_ese`
			...
			AND `ses_conf`.`id` = '1' 
			AND `marks_pr_ese`.`sesn_id` = `ses_conf`.`current_session` 
			AND `student`.`id` = `marks_pr_ese`.`stud_id` 
			AND `marks_pr_ese`.`course_id` = '$course' 
			AND `marks_pr_ese`.`exam_type` = '5' 
