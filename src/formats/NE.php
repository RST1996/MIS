...
if( isset($_GET['id']) && (isLogin() != null) )
{
		...
		else
		{
			...
			$exam = $_GET['exam'];
			$data_query = "SELECT  `academic_year`.`academic_year` ,  `course`.`course_code` ,  `course`.`course_name` ,  `course`.`credits` ,  `course_assign`.`sem` ,  `course_assign`.`branch` ,`department`.`department_name` ,  `exam_session`.`session`,`year2sem_relation`.`year`,`exam_table`.`exam_name_abvr`
			FROM  `exam_session`,`year2sem_relation` ,  `ses_conf` ,  `academic_year` ,  `course` ,`exam_table` ,  `course_assign` 
...
// Title
$pdf->Cell(190,7,"Not Elligible Students List for Summer 2017",0,1,'C',true);
...
// Line break
	...
	$sesn_id = get_sesn_id();
			$query = "
					SELECT  `ne_list`.`stud_id` ,`student`.`prn` ,`student`.`name` ,`ne_list`.`reason` 
					FROM  `ne_list` ,`student` ,`ses_conf` 
					WHERE  `ne_list`.`stud_id` =  `student`.`id` 
					AND  `ses_conf`.`id` =  '1'
					AND  `ne_list`.`sesn_id`  = '$sesn_id'
					AND  `ne_list`.`exam_id` =  '$exam'
					AND  `ne_list`.`course_id` =  '$course'
