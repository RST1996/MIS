...
if( isset($_GET['id']) && (isLogin() != null) )
{
		...
		else
		{
			...
			$data_query = "SELECT  `academic_year`.`academic_year` ,  `course`.`course_code` ,  `course`.`course_name` ,  `course`.`credits` ,  `course_assign`.`sem` ,  `course_assign`.`branch` , `th_mks_scheme`.`ese` ,  `department`.`department_name` ,  `exam_session`.`session`,`year2sem_relation`.`year`
			...
			$fileName = 'R05 ' . $row2['course_code'] . '.pdf';
		...
		// Line break
			...
			$pdf->SetFont('');
			//while start
			$query = "SELECT `student`.`id`,`student`.`prn`,`student`.`name`,`student`.`department`,`student`.`current_semester`,`marks_th_ese`.`marks`,`marks_th_ese`.`ne_flag`,`marks_th_ese`.`ab_flag`,`course`.`course_code`,`course`.`course_name` FROM `student`,`course_assign`,`course`,`exam_registration`,`ses_conf`,`marks_th_ese`
			WHERE `student`.`status` = 'ONGOING' 
			AND `course_assign`.`sub_id` = '$course' 
			AND `course_assign`.`sem` = `student`.`current_semester` 
			AND `course_assign`.`branch` = `student`.`department` 
			AND `exam_registration`.`stud_id` = `student`.`id` 
			AND `exam_registration`.`course_id` = `course_assign`.`sub_id` 
			AND `exam_registration`.`sesn_id` = `ses_conf`.`current_session` 
			AND `ses_conf`.`id` = '1' 
			AND `marks_th_ese`.`sesn_id` = `ses_conf`.`current_session` 
			AND `student`.`id` = `marks_th_ese`.`stud_id` 
			AND `marks_th_ese`.`course_id` = '$course' 
			AND `marks_th_ese`.`exam_type` = '5' 
			AND `exam_registration`.`conform_status` = '1' 
			AND `course`.`id` = '$course'  AND `student`.`department` = '$department'";
